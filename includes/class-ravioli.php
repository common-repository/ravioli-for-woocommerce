...
class Ravioli {
  ...
  private function define_admin_hooks() {
		...
		$this->loader->add_action( 'admin_enqueue_scripts', $plugin_admin, 'ravioli_enqueue_styles_and_scripts_admin' );
    ...
    // we add two different filters for the same method, one is legacy, one is HPOS compatible
    // CPT (legacy)
    $this->loader->add_filter( 'manage_edit-shop_order_columns', $plugin_admin, 'ravioli_new_order_column' );
    // HPOS
    $this->loader->add_filter( 'manage_woocommerce_page_wc-orders_columns', $plugin_admin, 'ravioli_new_order_column' );

    // we add two different actions for the same method, one is legacy, one is HPOS compatible
    // CPT (legacy)
    $this->loader->add_action( 'manage_shop_order_posts_custom_column', $plugin_admin, 'ravioli_populate_column', 25, 2 );
    // HPOS
    $this->loader->add_action( 'manage_woocommerce_page_wc-orders_custom_column', $plugin_admin, 'ravioli_populate_column', 25, 2 );
    ...
    $this->loader->add_action( 'woocommerce_checkout_after_customer_details', $plugin_public, 'ravioli_hidden_field');
